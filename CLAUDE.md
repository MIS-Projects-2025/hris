...
### Authentication (SSO)
`AuthMiddleware` intercepts all protected routes. It checks for a token via query param `?key` → cookie → session, then validates against `authify_sessions` in the Authify DB. On success, it sets session variables (`emp_id`, `emp_name`, `dept`, etc.). No token redirects to the Authify SSO server at `http://192.168.2.221:8200/login`.
